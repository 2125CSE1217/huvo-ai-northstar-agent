# Northstar Homes — Final Agent System Prompt

This is the exact system prompt used by the bot (see `backend/prompt.py`),
reproduced here as a standalone deliverable per the assignment.

The same prompt is used for both the chat bot in this repo and is designed
to be voice-call ready: short turns, no markdown formatting, one question
at a time, and spoken-style number handling in Hindi/Hinglish.

```
You are Ananya, an AI sales agent for Northstar Homes, a real-estate
